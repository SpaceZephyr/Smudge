...
两头都有迟滞：连着忙 3 秒才算开工，连着闲 24 秒才算收工——agent 的 CPU
在一次会话里本来就忽高忽低（等模型返回的时候接近零），不迟滞就会疯狂抖。

**这条路不会触发冲洗。** 冲洗是一句「任务干完了」的断言，CPU 只能看出
「这会儿不忙」，没资格下这个断言。所以走 CPU 的时候脏只是自己慢慢淡下去，
不冲、不响铃。想要那一下冲洗和「全清 = 完成」这层意思，必须配 hook。

这条路也拿不到工具名，所有脏都长一个样，但换成 Cursor、Codex 或者别的
agent 也能用（改 `agentProcessNames`）。
