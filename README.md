...
一个常驻菜单栏的 macOS 覆盖层。它解决两个问题，外加一个给手找事干的小玩意。
...
## 三、烧掉的 token 变成金币，晃鼠标吃

抹布是被动的——脏了才有得擦。金币是主动的。

Agent 每烧掉一批 token，屏幕上就冒出几枚金币，像个小喷泉。晃鼠标划过去就吃掉，连着吃音调会一路升上去。停着不动不算，得晃——这东西本来就是给手找事干的。

金币的数量对得上真实消耗：它读 Claude Code 的会话记录，按每条回复的 `output_tokens` 加 `cache_creation_input_tokens` 算，默认 400 token 一枚。所以屏幕上冒得越猛，就是这一轮烧得越狠。

这里有个我挺喜欢的别扭之处：**token 是你花的钱，但掉下来的时候它长得像奖励。** 烧得越多金币越多，捡得越开心。我没打算把这个感觉抹平，它本来就挺准确地描述了用 Agent 的体验。

菜单里可以关掉。参数在 `params.json` 里：`tokensPerCoin` 一枚值多少 token、`coinMagnet` 鼠标多近算吃到、`coinLife` 多久没人捡就消失。

![六种脏各来几块，中间横着擦了一道，右边飘着几枚金币](docs/preview.png)

上图是 `--selftest` 的离屏渲染：左上机械油污、中上积灰、右上哈气、左下咖啡渍、中下墨渍、右下水珠，中间那条横带是抹布走过的地方，右半边的斜纹是没擦匀留下的水痕，横着飘的那排是金币（最右边那枚正在被吃掉）。底下垫的是假桌面，用来看可读性。
...
TokenWatcher.swift  盯会话记录，把烧掉的 token 变成金币
